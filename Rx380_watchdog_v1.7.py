...
# Set up logging
logging.basicConfig(filename='rx380_logger_v1.7.log', level=logging.INFO,
...
class DataManager:
    ...
    def __init__(self, table_name='dbo.Office_Readings'):
        ...
        self.table_name = table_name  # Dynamically set table name

    async def save_to_sql(self, data_buffer):
        """Save data to SQL Server with error handling."""
        # Dynamically create the SQL insert query using the table name
        insert_query = f"""
        ...
        try:
            ...
            logging.info(f"Data inserted successfully into {self.table_name}! ({len(data_buffer)} records)")
        except Exception as e:
...
async def save_to_csv(data, folder_path=None):
    ...
    if folder_path is None:
        folder_path = Path.home() / "Desktop" / "P7_Oven" / "PUA" / "rx380_daily_logs"
...
async def main():
    ...
    data_manager = DataManager(table_name='dbo.P7_Oven')
