...
async def main():
    ...
    try:
        ...
        now = datetime.now()
        next_save_time = (now + timedelta(minutes=2 - now.minute % 2)).replace(second=0, microsecond=0)
        logging.info(f"Next data save scheduled at {next_save_time}")

        while True:
            # Wait until the next 2-minute mark
            now = datetime.now()
            ...
            if data:
                ...
                next_save_time = (datetime.now() + timedelta(minutes=2)).replace(second=0, microsecond=0)
                logging.info(f"Next data save scheduled at {next_save_time}")
